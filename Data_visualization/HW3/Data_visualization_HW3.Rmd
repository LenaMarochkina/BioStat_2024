...
df_init <- readRDS("very_low_birthweight.RDS") %>%
  mutate(ID = as.factor(row_number()),
    across(c(ID, apg1, twn, vent, pneumo, pda, cld, dead), ~ as.factor(.x))
  )
...
Plot density function plots for numeric variables. Remove outliers if any. Transform categorical variables into factors (did it when red the data). For any two numeric variables, color the plot by the variable ‘inout’.
...
PC1 explains 47.3% of the variance, the largest contribution among all components.
PC2 explains 20.3%.
Together, the first 2 components explain about 57.6% of the variance.
...
- pltct and lowph contribute to variance in PC2.
- longer arrows like hospstay and bwt indicate stronger influence.
...
# Prepare PCA data with patient_id
pca_data <- as.data.frame(pca_result$x)
pca_data$dead <- df$dead
pca_data$patient_id <- df$ID

# Interactive PCA biplot with plotly
p <- plot_ly(
  data = pca_data,
  x = ~PC1,
  y = ~PC2,
...
umap_config$n_neighbors <- 15  # the number of neighbors
umap_config$min_dist <- 0.2   # the minimum distance
...
grid.arrange(pca_plot, umap_plot, ncol = 2)
...
umap_config <- umap.defaults
umap_config$n_neighbors <- 20  # the number of neighbors
...
# Perform PCA on Original and Permuted Data
...
# Perform UMAP on Original and Permuted Data
...
pca_original_biplot <- ggplot(as.data.frame(pca_original$x), aes(x = PC1, y = PC2)) +
  geom_point(alpha = 0.7, aes(color = df_cleaned$dead)) +
...
# UMAP Visualization for Original and Permuted Data
...
# Perform correlation analysis
correlation_matrix <- cor(df_new_2, use = "complete.obs")
...
# Perform hierarchical clustering with the "ward.D2" method
res.hc <- hclust(df_cluster, method = "ward.D2")
...
When imputing missing values, more columns are retained in the analysis.

Complete case analysis ensures clarity and simplicity but risks data loss, while imputation preserves the dataset but may compromise the integrity of relationships in the data.
...
Additional clusters were identified with UMAP (4 instead of 2).