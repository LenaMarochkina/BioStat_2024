...
rearrange_columns <- function(data) {
  ...
  # Rearrange the remaining columns based on the desired order of suffixes
  ordered_columns <- unlist(lapply(desired_order, function(suffix) {
    grep(paste0(suffix, "$"), other_columns, value = TRUE)
  }))
  
  # Combine fixed columns and ordered columns
...
# Apply the function to df_transformed
...
# Define the exact suffixes of interest
...
# Function to calculate percentage of missing values by columns and remove those exceeding a threshold
...
df_cGVHD_prediction <- remove_cols_with_na(
...
Due to large ammount of immune cell markers (168):
...
# Identify and remove outliers using the three-sigma rule
...
# Apply the function to your dataset
df_diagnostic <- remove_cols_with_na(
  data = df_cGVHD_prediction
)
